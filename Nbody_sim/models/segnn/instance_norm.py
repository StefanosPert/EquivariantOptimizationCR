...
from torch_geometric.nn import global_mean_pool, global_max_pool #,aggr
...
class InstanceNorm(nn.Module):
    ...
    def __init__(self, irreps, eps=1e-5, affine=True, reduce='mean', normalization='component'):
        ...
        #self.pooling=aggr.MeanAggregation()
        if affine:
    ...
    def forward(self, input, batch):
        ...
        for mul, ir in self.irreps:  # mul is the multiplicity (number of copies) of some irrep type (ir)
            ...
            if ir.l == 0:
                ...
                #print("Max batch",torch.max(batch),torch.min(batch))
                #field_mean = self.pooling(field, batch,dim=0).reshape(-1, mul, 1)  # [batch, mul, 1]]
                # Subtract the mean
            ...
            if self.reduce == 'mean':
                ...
                #field_norm = self.pooling(field_norm, batch,dim=0)  # [batch, mul]
