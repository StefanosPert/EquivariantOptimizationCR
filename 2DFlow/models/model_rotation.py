...
class ApprConv(torch.nn.Module):
    def __init__(self,in_ch,out_ch,kernel_size):
        super(ApprConv,self).__init__()
        self.conv=torch.nn.Conv2d(in_ch,out_ch,kernel_size=kernel_size,padding=1,bias=False)
        self.mix_coef=1
        self.equiv=True
    def forward(self,x):
        if self.equiv:
            ...
            '''
            with torch.no_grad():
                rx=torch.cat([x.detach(),torch.rot90(x.detach(),k=1,dims=[2,3])],dim=0)
            w=self.conv(rx)
            n=torch.norm(w[0:w.shape[0]//2].reshape(-1))**2
            n_ld=torch.norm((w[0:w.shape[0]//2]-w[w.shape[0]//2:]).reshape(-1))**2
            '''
            return self.mix_coef*out,0,0 #,n_ld,n
...
class Relaxed_Rot_SteerConv(torch.nn.Module):
    def __init__(self, in_frames, out_frames, kernel_size, N, first_layer = False, last_layer = False):
        ...
        self.linear=ApprConv(self.relaxed_weights.shape[2]*self.basis_kernels.shape[2],self.relaxed_weights.shape[0]*self.basis_kernels.shape[1],kernel_size=kernel_size)
    ...
    def forward(self, x):
        ...
        #print("Conv filter",conv_filters.shape)
        #print("Relaxed weights",self.relaxed_weights.shape)
        #print("Basis kernels",self.basis_kernels.shape)
        #print("Prediction in",self.relaxed_weights.shape[0]*self.basis_kernels.shape[1])
        #print("Prediction out",self.relaxed_weights.shape[2]*self.basis_kernels.shape[2])
        #print("in ",x.shape)
        if not self.last_layer:

            out = F.conv2d(x, conv_filters, self.bias, padding = 1)+self.linear(x)[0]
            #print("Out",out.shape)
            return self.relu(e2cnn.nn.GeometricTensor(out, self.feat_type_hid)).tensor#self.norm(
        else:
            return F.conv2d(x, conv_filters, self.bias, padding = 1)+self.linear(x)[0]
