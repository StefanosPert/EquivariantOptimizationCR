...
class Relaxed_SESConv_Z2_H(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, effective_size,
        ...
        self.linear=ApprConv(self.in_channels,self.out_channels*self.num_scales,kernel_size=self.kernel_size)
    ...
    def forward(self, x):
        ...
        # convolution
        y = F.conv2d(x, kernel, bias=None, stride=self.stride, padding=self.padding)+self.linear(x)[0]
        B, C, H, W = y.shape
