...
parser.add_argument('--logdir',type=str,default='rotation_eq/')
...
for i in range(num_epoch):
    start = time.time()
    if i<=num_epoch/2:
            mix_coef=2*i/num_epoch
    else:
            mix_coef=1-2*(i-num_epoch/2)/num_epoch
    ...
    train_rmse.append(train_epoch(train_loader, model, optimizer, loss_fun,scale=scale))
    ...
    torch.save({"model": best_model.state_dict()}, 
                args.logdir+args.dataset + "_" + model_name +"_"+str(i)+ ".pt")
