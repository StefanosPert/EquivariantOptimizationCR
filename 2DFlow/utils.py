...
from models.model_rotation import ApprConv
from torch.utils import data
from models.model_scale_relaxed import Relaxed_Scale_GroupConv2d
...
def train_epoch(train_loader, model, optimizer, loss_function,scale=False):
    train_mse = []
    module_list=[]
    for module in model.modules():
        if isinstance(module,ApprConv):
    ...
    for xx, yy in tqdm(train_loader):
        ...
        for elem in module_list:
            weight=elem.conv.weight
            loss+=0.01*torch.norm(weight.reshape(-1))
            if scale:
                dil_weight=Relaxed_Scale_GroupConv2d.dilate_kernel(weight,1.5)
                loss+=0.01*torch.norm((weight-dil_weight).reshape(-1))
            else:
                loss+=0.01*torch.norm((weight-torch.rot90(weight,k=1,dims=[2,3])).reshape(-1))
        '''
        for module in model.modules():
            if isinstance(module,ApprConv):
